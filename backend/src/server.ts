...
async function convertPDFToJPG(inputPath: string): Promise<string[]> {
  try {
    ...
    try {
      ...
      if (popplerError.message && popplerError.message.includes("linux is NOT supported")) {
        ...
        for (let page = 1; page <= pageCount; page++) {
          const outputFile = path.join(outputDir, `${inputFileNameWithoutExt}-${page}.jpg`);
    ...
    const convertedFiles = files
      .filter(file => 
        file.startsWith(inputFileNameWithoutExt) && 
        file.endsWith('.jpg') && 
    ...
    for (let i = 0; i < convertedFiles.length; i++) {
      const inputFile = path.join(outputDir, convertedFiles[i]);
      const pageNumber = i + 1;
      const outputFile = path.join(outputDir, `${inputFileNameWithoutExt}_page_${pageNumber}.jpg`);
...
async function convertPDFToOffice(inputPath: string, outputFormat: 'docx' | 'xlsx' | 'pptx'): Promise<string> {
  try {
    const outputDir = tempDir;
    const inputFileName = path.basename(inputPath);
    const inputFileNameWithoutExt = path.basename(inputPath, path.extname(inputPath));
    ...
    const possibleOutputPaths = [
      path.join(outputDir, `${inputFileNameWithoutExt}.${outputFormat}`),
      path.join(outputDir, `input.${outputFormat}`), // In case LibreOffice uses the actual filename
      path.join(outputDir, `document.${outputFormat}`) // Default name sometimes used
    ];
    ...
    if (!outputFilePath) {
      try {
        ...
        const targetFiles = filesInDir.filter(file => file.endsWith(`.${outputFormat}`));
        if (targetFiles.length > 0) {
          outputFilePath = path.join(outputDir, targetFiles[0]);
...
async function convertPDFToWordWithPdf2docx(inputPath: string): Promise<string> {
  try {
    const outputDir = tempDir;
    const inputFileName = path.basename(inputPath);
    const inputFileNameWithoutExt = path.basename(inputPath, path.extname(inputPath));
    const outputFilePath = path.join(outputDir, `${inputFileNameWithoutExt}.docx`);
    ...
    const converterScriptPath = path.join(__dirname, '../ocr-service/pdf_to_word_converter.py');
    ...
    const { stdout, stderr } = await execAsync(pythonCommand, {
      ...
      cwd: path.dirname(converterScriptPath)
    });
...
async function detectPDFType(inputPath: string): Promise<'text-based' | 'image-based'> {
  try {
...
async function convertPDFWithOCR(inputPath: string, outputFormat: 'docx' | 'xlsx' | 'pptx'): Promise<string> {
  try {
    const outputDir = tempDir;
    const inputFileName = path.basename(inputPath);
    const inputFileNameWithoutExt = path.basename(inputPath, path.extname(inputPath));
    const outputFilePath = path.join(outputDir, `${inputFileNameWithoutExt}_ocr.${outputFormat}`);
    ...
    const ocrScriptPath = path.join(__dirname, '../ocr-service/pdf_ocr_converter.py');
    
    // Check if Python OCR script exists
    if (!await fs.pathExists(ocrScriptPath)) {
...
async function convertPDFToWordWithOCR(inputPath: string): Promise<string> {
  try {
    const outputDir = tempDir;
    const inputFileName = path.basename(inputPath);
    const inputFileNameWithoutExt = path.basename(inputPath, path.extname(inputPath));
    const outputFilePath = path.join(outputDir, `${inputFileNameWithoutExt}_ocr.docx`);
...
app.patch('/api/admin/users/:userId/role', authenticate, adminOnly, async (req, res) => {
  try {
