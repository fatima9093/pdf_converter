...
router.get('/conversions', authenticate, adminOnly, async (req, res) => {
  try {
...
router.get('/file-records', authenticate, adminOnly, async (req, res) => {
  try {
...
function transformFileType(dbFileType: string): string {
...
function transformFileStatus(dbStatus: string): string {
  const mapping: Record<string, string> = {
    'PENDING': 'Pending',
...
function transformToolToFileType(toolType: string): string {
  const mapping: Record<string, string> = {
    'word-to-pdf': 'Word',
    'excel-to-pdf': 'Excel',
...
function transformConversionStatus(status: string): string {
  const mapping: Record<string, string> = {
    'PENDING': 'Pending',
