...
export default function FileUploadModal({ tool, isOpen, onClose }: FileUploadModalProps) {
  ...
  useEffect(() => {
    if (isOpen && !isAuthenticated) {
      ...
      if (!limitResult.canConvert) {
        console.log('🚫 Conversion limit reached on modal open, redirecting to signup');
  ...
  const handleProcess = async () => {
    ...
    if (!limitResult.canConvert) {
      console.log('🚫 Conversion limit reached, redirecting to signup');
    ...
    try {
      if (isMultiFileMode && uploadedFiles.length > 1) {
        ...
        if (response.ok) {
          ...
          // Increment conversion count for anonymous users AFTER successful conversion
          if (!isAuthenticated) {
        ...
        if (result.success && result.data) {
          ...
          (window as unknown as { lastConversionBlobType: string }).lastConversionBlobType = blob.type;
  ...
  const handleDownload = () => {
    if (downloadUrl && uploadedFiles.length > 0) {
      ...
      if (isMultiFileMode) {
        ...
        const blobType = (window as unknown as { lastConversionBlobType: string }).lastConversionBlobType;
  ...
  const resetModal = () => {
    ...
    delete (window as unknown as { lastConversionBlobType?: string }).lastConversionBlobType;
  };
