...
import { X, Upload, File, Download, AlertCircle, CheckCircle, Plus, Trash2, FileText } from 'lucide-react';
...
export default function SplitPDFModal({ tool, isOpen, onClose }: SplitPDFModalProps) {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Split operation failed');
      }
    } catch (err) {
        ...
        <div className="p-6">
          ...
          {uploadedFile && pdfPageCount > 0 && !isCompleted && (
            <div className="space-y-6">
              ...
              {splitMode === 'individual' && pdfPageCount > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-blue-700 text-sm">
                    <strong>Preview:</strong> This will create {pdfPageCount} separate PDF files, one for each page 
                    (Page1.pdf, Page2.pdf, ..., Page{pdfPageCount}.pdf).
                  </p>
                </div>
              )}
